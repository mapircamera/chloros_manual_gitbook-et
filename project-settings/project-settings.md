...
### Minimaalne kalibreerimise proovi pindala (px)
...
* **Millal keelata**: Keelake ainult juhul, kui teie kaamera/objektiivi kombinatsioon on juba rakendanud vignettide korrigeerimist või kui soovite vignette käsitsi järeltöötluses korrigeerida.

### Peegelduskalibreerimine / valgebalanss

* **Tüüp**: Valikukast
* **Vaikimisi**: Lubatud (märgitud)
* **Kirjeldus**: võimaldab automaatset peegelduskalibreerimist, kasutades piltides tuvastatud kalibreerimise sihtmärke. See normaliseerib peegeldusväärtused kogu andmekogus ja tagab ühtlased mõõtmised olenemata valgustingimustest.
...
* **Millal kohandada**:
  ...
  * Suurendage väärtust (nt 60–300 sekundini), et saavutada kiirem töötlemine, kui valgustingimused on ühtlased ja teil on sagedased kalibreerimise sihtmärgi pildid.
...
* **Vaikimisi**: 0 tundi
* **Kirjeldus**: Määrab valgussensori andmete ajamärgete ajavööndi nihke (tundides UTC-st). Seda kasutatakse PPK (Post-Processed Kinematic) andmefailide töötlemisel, et tagada piltide salvestamise ja GPS-andmete õige ajasünkroniseerimine.
...
* **Saadaval olevad indeksid**: Süsteem sisaldab üle 30 eelmääratletud multispektraalset indeksit, sealhulgas:
...
* **Kirjeldus**: Võimaldab luua ja salvestada kohandatud multispektraalsete indeksite valemeid, kasutades riba matemaatikat. Kohandatud valemid salvestatakse koos projekti seadetega ja neid saab kasutada nagu sisseehitatud indekseid.
* **Kuidas luua**:
  1. Indeksi konfiguratsiooni paneelis otsige kohandatud valemi valik.
  2. Määrake valem, kasutades sagedusriba identifikaatoreid (nt NIR, Red, Green, Blue).
  3. Salvesta valem kirjeldava nimega.
* **Valemi süntaks**: Toetatakse standardseid matemaatilisi operatsioone, sealhulgas:
  * Aritmeetika: `+`, `-`, `*`, `/`
  * Sõlmete järjekorra sulgudes
...
* **Tüüp**: rippmenüü
* **Valikud**:
  * **TIFF (16-bitine)** – tihendamata 16-bitine TIFF formaat
...
* **Vormingu soovitused**:
  * **TIFF (16-bitine)**: Soovitatav teaduslikuks analüüsiks ja professionaalseteks töövoogudeks. Säilitab maksimaalse andmekvaliteedi ilma kompressiooniartefaktideta. Sobib kõige paremini multispektraalsele analüüsile ja edasisele töötlemisele GIS-tarkvaras.
  ...
  * **PNG (8-bitine)**: Sobib veebivaatamiseks ja üldiseks visualiseerimiseks. Väiksemad failisuurused kaotuseta kompressiooniga, kuid väiksem dünaamiline ulatus.
  * **JPG (8-bitine)**: Väikseimad failisuurused, sobib ainult eelvaatamiseks ja veebis kuvamiseks. Kasutab kaotusega kompressiooni, mis ei sobi teaduslikuks analüüsiks.
...
* **Kirjeldus**: Sisestage oma seademalli kirjeldav nimi ja klõpsake salvestamise ikooni. Mall salvestab kõik praegused projekti seaded (sihtmärgi tuvastamine, töötlemisvalikud, indeksid ja ekspordivorming), et neid saaks tulevastes projektides hõlpsasti uuesti kasutada.
* **Kasutusjuhtumid**:
  * Looge mallid erinevatele kaamerasüsteemidele (RGB, multispektraalne, NIR)
  * Salvesta standardkonfiguratsioonid konkreetsete põllukultuuride või analüüsi töövoogude jaoks
...
* **Kirjeldus**: Näitab praegust vaikimisi kataloogi, kuhu uued Chloros projektid loodakse. Klõpsake redigeerimisikooni, et valida teine kataloog.
* **Millal muuta**:
...
4. **Käsitsi tehtud muudatused** – kõik muudatused, mis teete käesoleva seansi jooksul

### Seaded ja pilditöötlus

Enamik seadeid (eriti töötlemise ja ekspordi kategooriates) käivitab piltide uuesti töötlemise, et kajastada uusi seadeid. Mõned seaded on aga „ainult ekspordiks” ja ei vaja kohest uuesti töötlemist:
...
3. **Testige enne täielikku töötlemist**: kui katsetate uusi seadeid, testige neid väikese pildivaliku peal enne kogu andmekogumi töötlemist.
